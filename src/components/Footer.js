"use client";

import Link from 'next/link';
import NewsletterSignupForm from './NewsletterSignupForm';

export default function Footer({ fullPage = false }) {
  return (
    <footer 
      className={`${fullPage ? "h-screen" : ""} bg-black text-white`}
      style={{
        // CRITICAL: Prevent layout shifts during scroll
        position: 'relative',
        width: '100%',
        minHeight: fullPage ? '100vh' : 'auto',
        // Tell browser this won't affect other elements
        contain: 'layout style paint',
        // Prevent transforms from affecting children
        transformStyle: 'flat',
      }}
    >
      <div 
        className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${fullPage ? "h-full flex flex-col justify-center" : "py-16"}`}
        style={{
          // Optimize container
          willChange: 'auto',
        }}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="flex justify-between">
            {/* About Column */}
            <div>
              <h3 className="text-sm font-semibold mb-4">About</h3>
              <ul className="space-y-2 text-sm text-gray-300">
                <li>
                  <Link 
                    href="/blog" 
                    className="hover:text-white"
                    style={{ transition: 'color 0.2s ease' }}
                  >
                    Blog
                  </Link>
                </li>
                <li>
                  <Link 
                    href="#" 
                    className="hover:text-white"
                    style={{ transition: 'color 0.2s ease' }}
                  >
                    Our Story
                  </Link>
                </li>
                <li>
                  <Link 
                    href="#" 
                    className="hover:text-white"
                    style={{ transition: 'color 0.2s ease' }}
                  >
                    Careers
                  </Link>
                </li>
              </ul>
            </div>

            {/* Customer Service Column */}
            <div>
              <h3 className="text-sm font-semibold mb-4">Customer Service</h3>
              <ul className="space-y-2 text-sm text-gray-300">
                <li>
                  <Link 
                    href="/returns" 
                    className="hover:text-white"
                    style={{ transition: 'color 0.2s ease' }}
                  >
                    Returns & Exchanges
                  </Link>
                </li>
                <li>
                  <Link 
                    href="/contact" 
                    className="hover:text-white"
                    style={{ transition: 'color 0.2s ease' }}
                  >
                    Contact Us
                  </Link>
                </li>
              </ul>
            </div>
          
            {/* Follow Us Column */}
            <div>
              <h3 className="text-sm font-semibold mb-4">Follow Us</h3>
              <div className="flex space-x-4">
                <Link 
                  href="#" 
                  className="text-gray-300 hover:text-white"
                  style={{ 
                    transition: 'color 0.2s ease',
                    display: 'inline-block',
                  }}
                  aria-label="Facebook"
                >
                  <svg 
                    className="w-5 h-5" 
                    fill="currentColor" 
                    viewBox="0 0 24 24"
                    style={{
                      // Prevent SVG from triggering repaints
                      pointerEvents: 'none',
                    }}
                  >
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </Link>
                <Link 
                  href="#" 
                  className="text-gray-300 hover:text-white"
                  style={{ 
                    transition: 'color 0.2s ease',
                    display: 'inline-block',
                  }}
                  aria-label="Instagram"
                >
                  <svg 
                    className="w-5 h-5" 
                    fill="currentColor" 
                    viewBox="0 0 24 24"
                    style={{
                      pointerEvents: 'none',
                    }}
                  >
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.058 1.644-.07 4.849-.07zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                  </svg>
                </Link>
              </div>
            </div>
          </div>

          {/* Newsletter Section */}
          <div className="mb-8 md:mb-12 text-center">
            <h3 className="text-xl md:text-2xl font-semibold mb-4">Stay Updated</h3>
            <p className="text-gray-300 mb-6 max-w-md mx-auto text-sm md:text-base">
              Subscribe to our newsletter for exclusive updates, new arrivals, and special offers delivered straight to your inbox.
            </p>
            <NewsletterSignupForm />
          </div>
        </div>

        {/* Bottom Section */}
        <div className="md:mt-12 pt-6 md:pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center">
          <div className="text-sm text-gray-400">
            © 2025 Rever. All rights reserved.
          </div>
          <div className="mt-4 md:mt-0">
            <Link 
              href="/" 
              className="text-xl font-semibold text-white"
              style={{ transition: 'opacity 0.2s ease' }}
            >
              Rever
            </Link>
          </div>
        </div>
      </div>

      <style jsx>{`
        /* Remove Tailwind's transition-colors - use color transitions instead */
        a {
          transition-property: color, opacity;
          transition-duration: 0.2s;
          transition-timing-function: ease;
        }
        
        /* Optimize hover states - no layout changes */
        a:hover {
          text-decoration: none;
        }
        
        /* Prevent text selection during scroll (iOS optimization) */
        footer {
          -webkit-user-select: none;
          user-select: none;
        }
        
        /* Re-enable for forms/inputs */
        input, textarea, button {
          -webkit-user-select: auto;
          user-select: auto;
        }
      `}</style>
    </footer>
  );
}